...
class PhilipsHomeAccessAPI:
    ...
    def login(self):
        ...
        _LOGGER.debug(
            "Login start: username=%s region=%s",
            self._mask(self.username),
            self.region_code,
        )
        ...
        _LOGGER.debug(
            "Login succeeded but requested region was not found: region=%s",
            self.region_code,
        )
    ...
    def get_devices(self):
        ...
        _LOGGER.debug(
            "get_devices start: uid=%s token_present=%s",
            self._mask(self.uid),
            bool(self.token),
        )
    ...
    def set_auto_lock_mode(self, esn, enabled):
        ...
        _LOGGER.debug(
            "set_auto_lock_mode start: esn=%s enabled=%s amMode=%s",
            self._mask(esn),
            enabled,
            mode,
        )
    ...
    def set_auto_lock_time(self, esn, seconds):
        ...
        _LOGGER.debug(
            "set_auto_lock_time start: esn=%s seconds=%s",
            self._mask(esn),
            int(seconds),
        )

        resp = requests.post(
            url, headers=self._get_headers(), json=final_body, timeout=10
        )
        out = resp.json()
        _LOGGER.debug(
    ...
    def set_lock_state(self, esn, lock_it):
        ...
        try:
            ...
            _LOGGER.debug(
                "set_lock_state returned non-JSON response: http_status=%s",
                resp.status_code,
            )
            return {
                "code": resp.status_code,
